...
export async function enrollInCourse(
  ...
  if (authError || !user) {
    return {
      error: "Inicia sesión para inscribirte.",
      success: false,
    };
  }
  ...
  if (
    ...
    return {
      error: "Tu perfil no tiene permiso para inscribirse.",
      success: false,
    };
  }
  ...
  // La clave primaria compuesta evita duplicados.
  // Nunca se acepta un ID de estudiante enviado desde el formulario.
  const { error } = await db.from("course_enrollments").insert({
    course_id: course.id,
    student_id: user.id,
  });

  // 23505 = ya existe la inscripción.
  if (error && error.code !== "23505") {
    return {
      error: "No se pudo completar la inscripción. Intenta de nuevo.",
      success: false,
    };
  }
